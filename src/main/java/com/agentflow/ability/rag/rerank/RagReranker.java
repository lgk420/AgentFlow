 ...
 *
 * <p><b>为什么需要它</b>：{@link RagRetriever} 走的向量检索是 bi-encoder——查询与文档
 ...
 * （复用 {@link RagRetriever} 与 {@code LlmClient} 的同款套路）。
 */
public interface RagReranker {
     ...
     * @param ragChunks 候选块，通常来自向量检索（入参顺序即向量序）
     * @param topN      重排后保留几条
     ...
     */
    List<RagChunk> rerank(String query, List<RagChunk> ragChunks, int topN);
}
